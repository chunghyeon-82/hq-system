...
import { useSettings, FontSize, AlertType } from '@/lib/settings-context'
...
export default function SettingsPage() {
  ...
  const alertTypes: { value: AlertType; label: string; desc: string }[] = [
    ...
    { value: 'vibrate',       label: '진동만',      desc: '소리 없이 진동만 울립니다' },
    { value: 'off',           label: '알림 끄기',   desc: '모든 알림을 받지 않습니다' },
  ]
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          ...
          {true && (
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              ...
              {alertTypes.map(t => (
                <button key={t.value}
                  onClick={() => updateSettings({ alertType: t.value })}
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                    settings.alertType === t.value ? 'border-primary-600 bg-primary-600' : 'border-gray-300'
                  }`}>
                    {settings.alertType === t.value && (
