...
import {
  ...
  ChevronLeft, ChevronRight, MapPin, Clock, Share2, Users, Calendar
} from 'lucide-react'
...
const COLORS = [
  { value: 'purple', bg: 'bg-primary-100',  text: 'text-primary-800', bar: 'bg-primary-400',  label: '보라' },
  { value: 'amber',  bg: 'bg-amber-100',    text: 'text-amber-800',   bar: 'bg-amber-400',    label: '주황' },
  { value: 'blue',   bg: 'bg-blue-100',     text: 'text-blue-800',    bar: 'bg-blue-400',     label: '파랑' },
  { value: 'green',  bg: 'bg-green-100',    text: 'text-green-800',   bar: 'bg-green-400',    label: '초록' },
  { value: 'red',    bg: 'bg-red-100',      text: 'text-red-800',     bar: 'bg-red-400',      label: '빨강' },
  { value: 'gray',   bg: 'bg-gray-100',     text: 'text-gray-700',    bar: 'bg-gray-400',     label: '회색' },
]
const COLOR_MAP: Record<string, { bg: string; text: string; bar: string }> = Object.fromEntries(
  COLORS.map(c => [c.value, { bg: c.bg, text: c.text, bar: c.bar }])
)
...
// 기간 일정 유틸
const dateInRange = (dateStr: string, startStr: string, endStr?: string) => {
  if (!endStr || endStr === startStr) return dateStr === startStr
  return dateStr >= startStr && dateStr <= endStr
}
const isRangeStart  = (dateStr: string, e: CalendarEvent) => dateStr === e.date
const isRangeEnd    = (dateStr: string, e: CalendarEvent) => e.endDate ? dateStr === e.endDate : dateStr === e.date
const isRangeMiddle = (dateStr: string, e: CalendarEvent) =>
...
export default function CalendarPage() {
  ...
  const eventsOnDate = (dStr: string) =>
    events
      .filter(e => !e.isDone && dateInRange(dStr, e.date, e.endDate))
      .sort((a, b) => {
        const aRange = !!(a.endDate && a.endDate !== a.date)
        const bRange = !!(b.endDate && b.endDate !== b.date)
  ...
  const openEditModal = (e: CalendarEvent) => {
    ...
    const hasRange = !!(e.endDate && e.endDate !== e.date)
    setIsRange(hasRange)
    ...
    setReminder(e.reminder ? `${e.reminder.value}:${e.reminder.unit}` : '')
    setModal(true)
  }
  ...
  const handleSave = async () => {
    ...
    if (isRange && endDate && endDate < selectedDate) {
      setSaveError('종료일이 시작일보다 빠릅니다'); return
    }
    ...
    try {
      const data: Omit<CalendarEvent, 'id' | 'createdAt'> & { color?: string } = {
        title:      title.trim(),
        date:       selectedDate,
        ...(isRange && endDate && endDate !== selectedDate ? { endDate } : {}),
        time:       (allDay || isRange) ? undefined : getTime24(),
  ...
  const handleDelete = async (id: string) => {
    ...
    await deleteEvent(id); setModal(false)
  }
            ...
            {pending.map(e => (
              <div key={e.id} className="bg-primary-50 border border-primary-200 rounded-xl p-4">
                ...
                <p className="text-sm text-primary-700 mb-3">
                  "{e.title}" — {formatDateKo(e.date)}
                  {e.endDate && e.endDate !== e.date ? ` ~ ${formatDateKo(e.endDate)}` : ''}
                  {e.time ? ` ${e.time}` : ''}
              ...
              {importantEvents.map(e => {
                ...
                return (
                  <button key={e.id} onClick={() => openEditModal(e)}
                    ...
                    <div className={clsx('w-2 h-2 rounded-full shrink-0', c.bar)}/>
                    ...
                    {(e.sharedWith?.length ?? 0) > 0 && <Share2 size={12} className="text-primary-400 shrink-0"/>}
                  </button>
                )
              })}
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-semibold text-gray-900 min-w-[100px] text-center">{viewYear}년 {viewMonth+1}월</h3>
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: lastDate }, (_, i) => {
              ...
              const lunar  = lunarDates[dStr] ?? ''

              return (
                <div key={d}
                  className={clsx(
                    'min-h-[90px] border-r border-b border-gray-50 p-1 cursor-pointer transition-colors',
                    isToday ? 'bg-primary-50/40' : isRed ? 'bg-red-50/30' : 'hover:bg-gray-50',
                ...
                >
                  ...
                  <div className="flex flex-col items-start mb-0.5">
                    <div className={clsx('w-6 h-6 flex items-center justify-center text-xs font-bold rounded-full',
                      isToday ? 'bg-primary-600 text-white' : isRed ? 'text-red-500' : 'text-gray-800')}>
                      {d}
                    </div>
                    {holidayName
                      ? <span className="text-[9px] text-red-500 font-medium leading-tight mt-0.5 truncate w-full">{holidayName}</span>
                      : lunar ? <span className="text-[9px] text-gray-400 leading-tight mt-0.5">{lunar}</span>
                      : null}
                  </div>
                  ...
                  <div className="space-y-0.5">
                    {dayEvents.slice(0, 3).map(ev => {
                      ...
                      if (isMultiDay) {
                        const start = isRangeStart(dStr, ev)
                        const end   = isRangeEnd(dStr, ev)
                        const mid   = isRangeMiddle(dStr, ev)
                        return (
                          <div key={ev.id}
                            className={clsx(
                              'text-xs h-5 flex items-center cursor-pointer overflow-hidden',
                              c.bar,
                              start && !end  ? 'rounded-l-full pl-2 pr-0 mr-0' : '',
                              end   && !start ? 'rounded-r-full pr-2 pl-0 ml-0' : '',
                              start && end   ? 'rounded-full px-2' : '',
                              mid            ? 'rounded-none px-0' : ''
                            )}
                          ...
                          >
                            {start && (
                              <span className="text-white text-[10px] font-semibold truncate leading-none whitespace-nowrap">
                                {ev.title}
                              </span>
                            )}
                          </div>
            ...
            <div className="bg-white border border-gray-100 rounded-xl divide-y divide-gray-50">
              {events.filter(e => e.isDone).slice(0, 5).map(e => (
                <div key={e.id} className="flex items-center gap-3 px-4 py-2.5 opacity-50">
                  <button onClick={() => toggleDone(e)}><CheckCircle2 size={16} className="text-green-400"/></button>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="p-5 space-y-4">
                  ...
                  {COLORS.map(c => (
                    <button key={c.value} onClick={() => setColor(c.value)}
                      className={clsx('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border transition-colors',
                        c.bg, c.text, color===c.value ? 'ring-2 ring-offset-1 ring-primary-400' : 'opacity-70')}>
              ...
              {/* 기간 일정 토글 */}
              <div>
                <button onClick={() => { setIsRange(v => !v); if (!isRange) setAllDay(true) }}
                  className={clsx('w-full flex items-center gap-3 px-4 py-3 border rounded-xl transition-colors',
                    isRange ? 'bg-blue-50 border-blue-300' : 'bg-white border-gray-200 hover:bg-gray-50')}>
                  <Calendar size={16} className={isRange ? 'text-blue-500' : 'text-gray-400'}/>
                  <div className="flex-1 text-left">
                    <p className={clsx('text-sm font-medium', isRange ? 'text-blue-800' : 'text-gray-700')}>
                      기간 일정
                    </p>
                    <p className="text-xs text-gray-400">며칠에 걸친 일정을 입력합니다</p>
                  </div>
                  <div className={clsx('w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0',
                    isRange ? 'border-blue-500 bg-blue-500' : 'border-gray-300')}>
                    {isRange && <span className="text-white text-xs font-bold">✓</span>}
                  </div>
                </button>
                  ...
                  <div className="mt-2 bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className="flex-1">
                        <p className="text-xs text-blue-600 font-medium mb-1">시작일</p>
                        <p className="text-sm font-semibold text-blue-900">{formatDateKo(selectedDate)}</p>
                      </div>
                      <span className="text-blue-400 text-lg">→</span>
                      <div className="flex-1">
                        <p className="text-xs text-blue-600 font-medium mb-1">종료일</p>
                        <select value={endDate} onChange={e => setEndDate(e.target.value)}
                          ...
                          <option value={selectedDate}>당일 (하루)</option>
                          {endDateOptions.map(d => {
                            const dt = new Date(d + 'T00:00:00')
                            const label = dt.toLocaleDateString('ko-KR', { month:'long', day:'numeric' })
                            const days = Math.round((new Date(d).getTime() - new Date(selectedDate).getTime()) / 86400000)
                            return <option key={d} value={d}>{label} ({days}일간)</option>
                          })}
                        </select>
                      </div>
                    </div>
                    {endDate && endDate !== selectedDate && (
                      <p className="text-xs text-blue-700 text-center font-medium">
                        📅 {formatDateKo(selectedDate)} ~ {formatDateKo(endDate)}
                      </p>
                    )}
                  </div>
                )}
              ...
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <label className="text-sm font-medium text-gray-700">장소 <span className="text-xs text-gray-400 font-normal">(선택)</span></label>
                </div>
              ...
              {/* 내용 */}
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-1.5">내용 <span className="text-xs text-gray-400 font-normal">(선택)</span></label>
                <textarea value={memo} onChange={e => setMemo(e.target.value)}
                  placeholder="메모" rows={3}
                  ...
                  <div className="mt-2 border border-primary-100 rounded-xl overflow-hidden">
                    ...
                    {shareTargets.map(u => (
                      <button key={u.uid} onClick={() => toggleShareUser(u.uid)}
                        className={clsx('w-full flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0 transition-colors',
              ...
              {saveError && (
                <p className="text-sm text-red-500 bg-red-50 border border-red-200 rounded-xl px-4 py-3">{saveError}</p>
              )}
