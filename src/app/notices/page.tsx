...
export default function NoticesPage() {
  ...
  const canWrite = user?.role === 'ADMIN' || user?.role === 'HQ_CHIEF' ||
                   user?.role === 'HQ_MEMBER' || isBizRep

  // 사업장 대표는 [사업장] 말머리만
  const prefixes: NoticePrefix[] = isBizRep ? ['사업장'] : ['본부', '사업장']

  // 사업장 대표의 사업장명
  const myBiz = businesses.find(b => b.id === user?.bizId)

  useEffect(() => {
    if (loading) return
    if (!user) { router.replace('/login'); return }
    const u1 = listenNotices(setNotices)
  ...
  const handleSubmit = async () => {
    ...
    await addNotice({
      ...
      ...(isBizRep && myBiz ? { bizName: myBiz.name } : {}),
          ...
          <div className="bg-white border border-primary-200 rounded-2xl p-5 space-y-4 shadow-sm">
            ...
            {isBizRep && myBiz && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 text-sm text-amber-700">
                📍 <strong>[사업장]</strong> 말머리로 등록됩니다 — {myBiz.name}
              </div>
            )}
            <div>
            ...
            <div>
              ...
              {showPicker && (() => {
                ...
                const applyDate = (y: number, mo: number, d: number, h: number, mi: number) => {
                  setExpiresAt(new Date(y, mo-1, d, h, mi).toISOString())
                }
                return (
                  <div className="mt-2 border border-primary-200 rounded-xl p-4 bg-white shadow-lg space-y-3">
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        ...
                        <select value={pickYear} onChange={e => { const y=+e.target.value; setPickYear(y); applyDate(y,pickMonth,pickDay,pickHour,pickMin) }}
                      ...
                      <div>
                        ...
                        <select value={pickMonth} onChange={e => { const mo=+e.target.value; setPickMonth(mo); applyDate(pickYear,mo,pickDay,pickHour,pickMin) }}
                          className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
                      ...
                      <div>
                        ...
                        <select value={pickDay} onChange={e => { const d=+e.target.value; setPickDay(d); applyDate(pickYear,pickMonth,d,pickHour,pickMin) }}
                          className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <select value={pickHour} onChange={e => { const h=+e.target.value; setPickHour(h); applyDate(pickYear,pickMonth,pickDay,h,pickMin) }}
                          className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
                      ...
                      <div>
                        ...
                        <select value={pickMin} onChange={e => { const mi=+e.target.value; setPickMin(mi); applyDate(pickYear,pickMonth,pickDay,pickHour,mi) }}
                          className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
